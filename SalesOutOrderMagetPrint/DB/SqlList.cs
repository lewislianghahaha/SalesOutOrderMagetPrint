...
namespace SalesOutOrderMagetPrint.DB
{
    public class SqlList
    {
        ...
        {
            _result = $@"SELECT CONVERT(VARCHAR(30),a.FDATE,111) 单据日期,a.FBILLNO 单据编号,
        ...
        {
            if (firstcustomerName == "" && seconCustomerName == "")
            {
                _result = $@"
            ...
            else if (firstcustomerName != "" && seconCustomerName=="")
            {
                _result = $@"
                            ...
                            WHERE (a.FDATE>='{sdt}')   --起始单据日期
                            AND (a.FDATE<='{edt}')     --结束单据日期
                            AND (x1.FNAME LIKE '%{firstcustomerName}%')         --一级客户名称
                            AND a.FDOCUMENTSTATUS='C'		--已审核
                            ORDER BY x1.FNAME,y1.FNAME,a.FDATE
                        ";
            }
            else if (firstcustomerName=="" && seconCustomerName!="")
            {
                _result = $@"
                            ...
                            WHERE (a.FDATE>='{sdt}')   --起始单据日期
                            AND (a.FDATE<='{edt}')     --结束单据日期
                            AND (y1.FNAME LIKE '%{seconCustomerName}%')      --二级客户名称
                            AND a.FDOCUMENTSTATUS='C'		--已审核
                            ORDER BY x1.FNAME,y1.FNAME,a.FDATE
                        ";
            }
